...
int TrendingTimer::interval() const {
    return m_interval;
}

void TrendingTimer::setInterval(int interval) {
    m_interval = interval;
}
...
void TrendingTimer::doRead() {
    ...
    m_item->clear();
    foreach (auto item, m_refes) {
        m_item->append(item->value().toReal());
    }
...
int TrendingTimer::timeLength() const {
    return m_timeLength;
}

void TrendingTimer::setTimeLength(int timeLength) {
