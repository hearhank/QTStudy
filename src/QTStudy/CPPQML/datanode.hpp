...
// typedef QVariant (*formatFunc)(QVariant& val);
class DataNode : public QObject {
  ...
  Q_PROPERTY(
      CH::DataConverter convertType READ convertType WRITE setConvertType)
  Q_PROPERTY(DataDesc* desc READ desc WRITE setDesc)
  Q_PROPERTY(DataCalc* calc READ calc WRITE setCalc)
  Q_PROPERTY(QString unit READ unit WRITE setUnit NOTIFY unitChanged)
  Q_PROPERTY(int pControlType READ pControlType WRITE setPControlType NOTIFY
                 pControlTypeChanged)
  Q_PROPERTY(bool enabled READ enabled WRITE setEnabled NOTIFY enabledChanged)
  //    Q_PROPERTY(bool store READ getStore WRITE setStore)
  Q_PROPERTY(bool hidden READ getHidden WRITE setHidden)
  Q_PROPERTY(
      QQmlListProperty<DataNode> children READ children NOTIFY childrenChanged)
  Q_PROPERTY(
      bool selected READ getSelected WRITE setSelected NOTIFY selectedChanged)
  Q_PROPERTY(
      QVariant fValue READ getFValue WRITE setFValue NOTIFY fValueChanged)
public:
  enum PControlType { Label, SWitchButton, ComboBox, TextBox, Button };
  Q_ENUM(PControlType)
  ...
  Q_INVOKABLE DataNode* getNode(const int index);
  QVariant getFValue() const;
