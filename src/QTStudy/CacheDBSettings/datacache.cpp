...
DataCache::DataCache(QObject *parent) :
  m_store("kvcnf","protonmdis",parent)
{
...
void DataCache::load(const QString &group) {
  m_load = false;
  QHash<QString, QHash<QString, QVariant> > datas = m_store.Load(group);
  if (group != "") {
    loadGroup(group, datas[group]);
  } else {
    foreach (QString gName, datas.keys()) {
      loadGroup(gName, datas[gName]);
    }
  }
  m_load = true;
}

void DataCache::loadGroup(const QString& group,
                          const QHash<QString, QVariant>& datas) {
  ...
  while (i.hasNext()) {
    auto item = i.next();
    m_datas[group].insert(item.key(), CacheItem(item.key(),item.value(),group));
  }
}
